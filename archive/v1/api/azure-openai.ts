...
type AzureOpenAIMode = 'analyze' | 'chat';
...
function buildSystemPrompt(mode: AzureOpenAIMode, blueprint?: Record<string, unknown>): string {
  if (mode === 'analyze') return ANALYZE_SYSTEM_PROMPT;
  return `${CHAT_SYSTEM_PROMPT}\n\nBlueprint context:\n${JSON.stringify(blueprint ?? {}, null, 2)}`;
}

function parseRequestBody(raw: unknown): AzureOpenAIRequestBody | null {
...
/**
 * Chat mode: standard streaming with Azure OpenAI.
 */
    ...
    async start(controller) {
      ...
      try {
        ...
        controller.enqueue(
          sseChunk({
            type: 'error',
            error: error instanceof Error ? error.message : 'Azure OpenAI stream failed',
          })
        );
        controller.close();
      }
...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (req.method === 'OPTIONS') {
    return new Response(null, {
      ...
      headers: {
        ...
        'Access-Control-Allow-Headers': 'content-type,x-api-key,x-azure-endpoint,x-azure-deployment',
  ...
  // Get Azure OpenAI configuration from headers
  const apiKey = req.headers.get('x-api-key')?.trim() ?? '';
  const endpoint = req.headers.get('x-azure-endpoint')?.trim() ?? '';
  const deploymentName = req.headers.get('x-azure-deployment')?.trim() ?? '';

  if (!apiKey) {
    return jsonResponse(401, {
      error: 'Missing Azure OpenAI API key. Provide it in the x-api-key request header.',
    });
  }

  if (!endpoint) {
    return jsonResponse(401, {
      error: 'Missing Azure OpenAI endpoint. Provide it in the x-azure-endpoint request header.',
    });
  }

  if (!deploymentName) {
    return jsonResponse(401, {
      error: 'Missing Azure OpenAI deployment name. Provide it in the x-azure-deployment request header.',
    });
  }
  ...
  try {
    ...
    // The endpoint should be: https://{resource}.openai.azure.com/openai/deployments/{deployment}
    let baseUrl = endpoint;
    // Remove trailing slash if present
    if (baseUrl.endsWith('/')) {
      baseUrl = baseUrl.slice(0, -1);
    }
    // Ensure the URL includes /deployments/{deployment-name}
    if (!baseUrl.includes('/deployments/')) {
      baseUrl = `${baseUrl}/deployments/${deploymentName}`;
    }
    // Add api-version query parameter (required for Azure OpenAI)
    const azureEndpoint = `${baseUrl}?api-version=2024-10-21`;

    console.log('[Azure OpenAI] Using endpoint:', azureEndpoint);
    ...
    const sse =
      body.mode === 'analyze'
        ? await analyzeWithAudio(openai, deploymentName, body)
        : await streamChatResponse(openai, deploymentName, body);
