...
export const AZURE_OPENAI_API_KEY_STORAGE_KEY = 'azure_openai_api_key';
export const AZURE_OPENAI_ENDPOINT_STORAGE_KEY = 'azure_openai_endpoint';
export const AZURE_OPENAI_DEPLOYMENT_STORAGE_KEY = 'azure_openai_deployment';

/** Files above 25 MB skip audio upload and fall back to text-only enrichment. */
const AZURE_OPENAI_AUDIO_LIMIT = 25 * 1024 * 1024;

interface AzureOpenAIEnhancement {
  groove?: string;
...
function parseEnhancement(raw: string): AzureOpenAIEnhancement | null {
  if (!raw || typeof raw !== 'string') return null;
  try {
    const parsed = JSON.parse(extractJson(raw)) as AzureOpenAIEnhancement;
...
function mergeEnhancement(
  ...
  enhancement: AzureOpenAIEnhancement | null
...
export function getStoredAzureOpenAIApiKey(): string | null {
  try {
    const key = localStorage.getItem(AZURE_OPENAI_API_KEY_STORAGE_KEY);
...
export function setStoredAzureOpenAIApiKey(key: string): void {
  try {
    const trimmed = key.trim();
    if (!trimmed) {
      localStorage.removeItem(AZURE_OPENAI_API_KEY_STORAGE_KEY);
      return;
    }
    localStorage.setItem(AZURE_OPENAI_API_KEY_STORAGE_KEY, trimmed);
  } catch {
    // no-op for non-browser contexts
  }
}

export function getStoredAzureOpenAIEndpoint(): string | null {
  try {
    const endpoint = localStorage.getItem(AZURE_OPENAI_ENDPOINT_STORAGE_KEY);
...
export function setStoredAzureOpenAIEndpoint(endpoint: string): void {
  try {
    const trimmed = endpoint.trim();
    if (!trimmed) {
      localStorage.removeItem(AZURE_OPENAI_ENDPOINT_STORAGE_KEY);
      return;
    }
    localStorage.setItem(AZURE_OPENAI_ENDPOINT_STORAGE_KEY, trimmed);
  } catch {
    // no-op for non-browser contexts
  }
}

export function getStoredAzureOpenAIDeployment(): string | null {
  try {
    const deployment = localStorage.getItem(AZURE_OPENAI_DEPLOYMENT_STORAGE_KEY);
...
export function setStoredAzureOpenAIDeployment(deployment: string): void {
  try {
    const trimmed = deployment.trim();
    if (!trimmed) {
      localStorage.removeItem(AZURE_OPENAI_DEPLOYMENT_STORAGE_KEY);
      return;
    }
    localStorage.setItem(AZURE_OPENAI_DEPLOYMENT_STORAGE_KEY, trimmed);
  } catch {
    // no-op for non-browser contexts
  }
}
...
function buildHeaders(): Record<string, string> {
  ...
  const key = getStoredAzureOpenAIApiKey();
  if (key) headers['x-api-key'] = key;
  
  const endpoint = getStoredAzureOpenAIEndpoint();
  if (endpoint) headers['x-azure-endpoint'] = endpoint;
  
  const deployment = getStoredAzureOpenAIDeployment();
...
function parseSsePayload(payload: string): {
  ...
  try {
    ...
    if (parsed.type === 'error') {
      return { done: true, error: parsed.error || 'Azure OpenAI stream error' };
    }
  } catch {
...
async function readErrorMessage(response: Response): Promise<string> {
  ...
  if (!text) return `Azure OpenAI API request failed (${response.status})`;
...
export class AzureOpenAIAnalysisProvider implements AnalysisProvider {
  name = 'Azure OpenAI GPT-4o (Cloud + Local DSP)';
  ...
  async isAvailable(): Promise<boolean> {
    ...
    const hasKey = getStoredAzureOpenAIApiKey();
    const hasEndpoint = getStoredAzureOpenAIEndpoint();
    const hasDeployment = getStoredAzureOpenAIDeployment();
    
    if (hasKey && hasEndpoint && hasDeployment) return true;

    try {
      const response = await fetch('/api/azure-openai', { method: 'GET' });
      return response.ok;
    } catch {
      return false;
    }
  ...
  async analyze(file: File): Promise<ReconstructionBlueprint> {
    ...
    if (file.size <= AZURE_OPENAI_AUDIO_LIMIT) {
      audioBase64 = await fileToBase64(file);
      audioMimeType = file.type || 'audio/mpeg';
    } else {
      console.warn(
        `[Azure OpenAI] File size ${(file.size / 1024 / 1024).toFixed(1)} MB exceeds 25 MB limit — skipping audio upload, text-only enrichment.`
      );
    }
    ...
    try {
      ...
      console.error('[Azure OpenAI] Analysis failed:', err);
      return {
        ...localBlueprint,
