...
export function parseClaudeEnhancement(raw: string): ClaudeEnhancement | null {
  if (!raw || typeof raw !== 'string') return null;
  try {
    const parsed = JSON.parse(extractJson(raw)) as ClaudeEnhancement;
...
export function getStoredAnthropicApiKey(): string | null {
  try {
    const key = localStorage.getItem(ANTHROPIC_API_KEY_STORAGE_KEY);
...
function parseSsePayload(payload: string): {
  ...
  try {
    ...
    if (parsed.type === 'error') {
      return { done: true, error: parsed.error || 'Claude stream error' };
    }
  } catch {
...
async function readErrorMessage(response: Response): Promise<string> {
  ...
  if (!text) return `Claude API request failed (${response.status})`;
...
export class ClaudeAnalysisProvider implements AnalysisProvider {
  ...
  async isAvailable(): Promise<boolean> {
    if (getStoredAnthropicApiKey()) return true;

    try {
      const response = await fetch('/api/claude', { method: 'GET' });
      return response.ok;
    } catch {
      return false;
    }
